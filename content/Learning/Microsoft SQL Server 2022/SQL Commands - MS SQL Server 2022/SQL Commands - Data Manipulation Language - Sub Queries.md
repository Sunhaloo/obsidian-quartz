...
>
> - SQL statement can have a ( _or multiple_ ) `SELECT` "_statements_" embedded within them
> - This "_sub-select_" can be used in `WHERE` and `HAVING` clauses of an **outer** `SELECT`, where it is called a **sub-query** or a **nested-query**.
> - Sub-selects may also appear in `INSERT`, `UPDATE`, and `DELETE` statements

> Let's get a real example going to be able to understand this "_nested-select_" statements.

## Examples of Sub-Queries

> Before I start did I explain ( _myself_ ) the `IN` keyword; because I think I have been using it...
...
> > Yes! **result** and <span style="color: red;"> not</span> _record_.
>
> ```csv
> branchNo
> B003
...
> > Like really how are you going to know how many values it will return ( _if you have an answer $\rightarrow$ please DM me!_ )
...
> These are going to be "_shit_" examples BTW.
...
>
> - `ANY` and `ALL` _may_ be used with **sub-queries** that produces a _single_ column of _numbers_
...
> [!WARNING]
> This $\downarrow$ example is redundant as the `ALL` keyword is performing the _same_ action as the `SELECT` statement ( _in this case_ ).
...
List the details of the staff whose salary is larger than the _salary_ of **every** member from the branch `B003`.
...
>
> > [!NOTE]
> > The question did not ask for "_any_" member of the same branch, that is `B003`.
> > Instead, who is the staff that has the largest salary <span style="color: red;"> compared</span> to the _salaries_ of the people at `B003`.
...
It will found find the smallest value for `salary` from the branch `B003` ( _like the sub-query_ ). Hence, in this case, the smallest value for `salary` is going to be `12000`.
...
> Where we have found most / any of the staff; where their salary is greater than the minimum salary of the _sub-query_.
...
>
> - Produces a simple `true` / `false` _result_
> - `EXISTS` $\downarrow$:
>   - `true` $\Rightarrow$ if and only if there exist **at least** one row in the table returned by the _sub-query_
>   - `false` $\Rightarrow$ if the _sub-query_ returns an **empty** result table
...
> We are now going to convert the above $\uparrow$ SQL Command with the `EXIST` keyword to a simple _inner_ ` JOIN` statement.
>
> ```SQL
> -- hence, "translated" to
