...
>
> > Because we are going to refer to some pages in it ( "*innit*" ).
...
> [!NOTE]-
> For the moment; I will only be adding the *simplest* `INSERT` statement's templates.
> This is because:
> 1. There are much more "*difficult*" ( *more like never saw them in my life* ) versions
...
In this case, the `INSERT` command, will check you table field ( *header* ) names / attributes and then will insert the values accordingly.
...
Suppose you have a simple 'Customer' table. Now, someone comes up to you and gives you 3 pages with many *values* to add into our table.
...
> [!NOTE]
> Please refer to '[[SQL Commands - Data Definition Language ( DDL )]' and '[[SQL Commands - Data Definition Language - Constraints]' if you are a bit "*washed*" or "*rusty*".

## Inserting Data Without Specifying Field Names

In this example, we are going to be using this '[[#Adding Values to All Columns]' *template*
...
Currently, our table 'Branch' does not have any values in **any** *fields*.
...
> Making these mistake to show *me, myself* and *I*
> What errors we are going to get.
> > "*A good programmer, understand the mistakes and then fixes the mistakes accordingly*"
>
...
> Hence, in the scenario, we are going to see what errors we are going to get when we do not add the *Primary Key* into the `INSERT` command
> > BTW I am **not** going to do every errors in the *world*... Obviously.
...
Now, we are going to try to add a record whereby the its *values* are **not** in the correct order of the field names in the table.
...
> [!TIP]
> *Again*, **Always** use this one!!!

This is pretty simple, again, here is the *template*.
...
When we are adding the values above $\uparrow$ ( *without specifying the column names* ). We are adding it in the order that the fields are in the actual table.
...
> But, it will facilitate your life as you only need to remember the field / attribute names ( *and not the order also* )
...
> Let's try adding this record into our table and then we can see if the insertion was in the correct *order*.
...
Now, we have tried some *possibilities*; let's go ahead and add the rest of the date using the following statements found in the code block below:
...
> I just found out that you don't need to run multiple `INSERT` commands ( *like $\uparrow$* ).
>
...
> ```
>
> > "*This is superior*!!!"... Thank You Karishma!
>

> [!INFO]
> You can just copy all of this, place it in the Query *Editor* and then press `<F5> ` to run the commands.
> But this *feature* does <span style="color: red;"> <strong> not</strong> </span> work in every SQL DBMSs.
> Hence, the lecturer always tells us to run it one at a time...
> > I do **not**. Coming from Oracle 10g ( *which was released in 2003* ) where we need to use it because the lecturer at [[University of Technology Mauritius Data View ( Level 1 Semester 1 ) | UTM] did not know any shit!
> > I will gladly use this "*run-all*" command.
...
> Hence in our case we ( *if you would please glance back at the above $\uparrow$ `.csv` values* ), we need to start at '2' because our **actual data** lives / starts at row '2'.
...
In fact, this *option* `FIELDTERMINATOR` is quite a jack of all trades; this is because it can take any character.
...
> [!NOTE]
> Sometimes you might see people use the "*character(s)*" `0x0a`. This is basically the same thing as `\n`.
> > [!INFO]
...
> > Basically Linux / Unix based system *end* lines of text in different format than Windows ( *because its shit $\rightarrow$ "for programmers"* )
...
> > ```
> > This is because Git needs to convert is to the most appropriate type of "*end of line sequence / character*".
> > 
...
We can also `INSERT` data from **another** *table* with the `SELECT` command.
...
>
> So the Lecturer told us to "*Populate the 'StaffPropCount' table using details from the 'Staff' and 'PropertyForRent' tables*"
> Hence, the *correct* answer for this example / question is:
> ```SQL
